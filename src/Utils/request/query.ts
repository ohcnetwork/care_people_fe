...
export async function callApi<Route extends ApiRoute<unknown, unknown>>(
  ...
  if (!res.ok) {
    ...
    // Run the plug's own handler so the user gets the structured error
    // message (e.g. validation `msg`s). Then mark the error `silent` so the
    // host's `mutationCache.onError` skips it — otherwise the host would
    // toast a generic "Request Failed" on top of our detailed toast, since
    // the host can't recognise the plug's `HTTPError` class via `instanceof`.
    if (!isSilent) {
      handleHttpError(httpError);
      httpError.silent = true;
    }
